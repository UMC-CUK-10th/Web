            ...
            <div className="animate-fade-in">
              <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-5 py-2 w-fit rounded-full transition-all active:scale-90 shadow-sm border ${
                ...
                {/* 태그 리스트: 스웨거의 "name" 속성 사용 */}
                <div className="flex flex-wrap gap-2">
                  {lp?.tags && lp.tags.length > 0 ? (
                    lp.tags.map((tag: any) => (
                      <span 
                        key={tag.id} 
                        className="px-3 py-1 bg-blue-50 text-blue-600 text-[11px] font-bold rounded-full border border-blue-100 shadow-sm"
                      >
                        #{tag.name}
                      </span>
                    ))
                  ) : (
                    <span className="text-gray-300 text-[11px]">#태그없음</span>
                  )}
                </div>
              </div>
