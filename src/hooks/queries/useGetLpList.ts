...
import type { PagnationDto } from '../../types/common';
...
function useGetLpList({ cursor, search, order, limit }: PagnationDto) {
  return useQuery({
    ...
    // 데이터가 신선하다고 간주하는 시간
    // 이 시간 동안은 캐시된 데이터를 그대로 사용합니다. 그리고 컴포넌트가 마운트 되거나 창에 포커스 들어오는 경우도 재요청 X
